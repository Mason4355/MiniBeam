...
const state = {
  ...
  inviteUrl: "",
  currentUrl: "",
  tabs: [{ id: "local-tab", title: "New tab", url: "", active: true }],
  socket: null
};
...
async function init() {
  ...
  ui.roomLabel.textContent = state.roomCode ? `Дом ${state.roomCode}` : "Подключение";
...
function bindSocket() {
  state.socket.on("room:state", (room) => {
    ...
    state.tabs = room.tabs?.length ? room.tabs : state.tabs;

    ui.roomLabel.textContent = `Дом ${room.roomCode}`;
    ui.inviteUrl.textContent = state.inviteUrl;
    ui.onlineLabel.textContent = `${room.participants?.length || 1} онлайн`;
...
function bindUi() {
  ...
  ui.copyInviteButtonFooter.addEventListener("click", () => copyInvite(ui.copyInviteButtonFooter, "Пригласить"));
  ...
  ui.newTabButton.addEventListener("click", () => {
    showHome();
    state.socket.emit("browser:new-tab");
  });
...
function renderTabs(tabs) {
  ...
  tabs.forEach((tab) => {
    ...
    button.innerHTML = `<span class="tab-favicon">◉</span>${escapeHtml(tab.title || "New tab")}`;
    ui.tabsList.append(button);
  });
}
...
function setTitle(title) {
  ui.tabTitle.textContent = title || "New tab";
  ui.tabTitle.title = title || "New tab";
}

function setBlockedCount(count) {
  ui.adblockText.textContent = String(count || 0);
}
